...
function gameAction(timestamp){
    ...
    //Add bugs
    if (timestamp - scene.lastBigSpawn > game.bugSpawnInterval + 5000 * Math.random()){
    ...
    clouds.forEach(element => {
        element.x -= game.speed;
        element.style.left = element.x + 'px';

        if (element.x + element.offsetWidth <= 0){
    ...
    bugs.forEach(bug => {
        ...
        fireBalls.forEach(element => {
            if (isCollision(element, bug)){
